...
if (isset($_POST['registrasi'])) {
    ...
    if ($pesan_error_user == "" && $pesan_error_foto == "" && $pesan_error_password == "" && $aturan_error == "" && $pesan_error_email == "") {
        ...
        $status_daftar = 1;
        $password_hash = password_hash($password_pembeli, PASSWORD_DEFAULT);

        // Ganti bagian ini sesuai dengan struktur tabel Anda
        $query = mysqli_query($conn, "INSERT INTO `pembeli` (`id_pembeli`, `username_pembeli`, `password_pembeli`, `nama_pembeli`, `jk_pembeli`, `alamat_pembeli`, `telp_pembeli`, `foto_pembeli`, `email_pembeli`, `status_pendaftaran`) VALUES (NULL, '$username_pembeli', '$password_hash', '$nama_pembeli', '$jk_pembeli', '$alamat_pembeli', '$telp_pembeli', '$namafoto', '$email_pembeli','$status_daftar')");
